...
func testMessagesExists(t *testing.T) {
	...
	e, err := MessageExists(ctx, tx, o.UUID)
	if err != nil {
...
func testMessagesFind(t *testing.T) {
	...
	messageFound, err := FindMessage(ctx, tx, o.UUID)
	if err != nil {
		t.Error(err)
	}
...
func testMessageToOneConversationUsingConversation(t *testing.T) {
	...
	queries.Assign(&local.ConversationID, foreign.UUID)
...
func testMessageToOneSetOpConversationUsingConversation(t *testing.T) {
	...
	for i, x := range []*Conversation{&b, &c} {
		...
		if !queries.Equal(a.ConversationID, x.UUID) {
		...
		if !queries.Equal(a.ConversationID, x.UUID) {
			t.Error("foreign key was wrong value", a.ConversationID, x.UUID)
		}
	}
}

func testMessageToOneRemoveOpConversationUsingConversation(t *testing.T) {
	var err error
...
var (
	messageDBTypes = map[string]string{`UUID`: `uuid`, `ConversationID`: `uuid`, `Role`: `character varying`, `Content`: `text`, `CreatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
